...
(defn part-1
  []
  (let [intersections (set/intersection (set (build-wire wire-1)) (set (build-wire wire-2)))]
    (->> (mapv #(calculate-manhattan-distance [0 0] %) intersections)
         (filter #(not (zero? %)))
         (apply min))))

(defn part-2
  []
  (let [wire-1 (build-wire wire-1)
        wire-2 (build-wire wire-2)
        intersections (set/intersection (set wire-1) (set wire-2))]
    (->> intersections
      (map #(calculate-wire-length wire-1 wire-2 %))
      (filter #(not (zero? %)))
      (apply min))))
